...
class TaxInvoice(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("tax_mode IN ('INTRA_STATE','INTER_STATE')", name="ck_invoice_tax_mode"),
        CheckConstraint(
            "status IN ('DRAFT','SUBMITTED','APPROVED','ISSUED','CANCELLED')",
            name="ck_invoice_status",
        ),
        Index("ix_tax_invoices_project", "project_id"),
        Index("ix_tax_invoices_loa", "loa_id"),
    )
...
class TaxInvoiceLine(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tax_invoice_id", "line_number", name="uq_tax_invoice_line_number"),
        CheckConstraint("invoiced_quantity > 0", name="ck_tax_invoice_line_quantity"),
        CheckConstraint("sales_rate >= 0", name="ck_tax_invoice_line_rate"),
        CheckConstraint(
            "NOT (loa_item_id IS NOT NULL AND variation_line_id IS NOT NULL)",
            name="ck_tax_invoice_line_single_contract_source",
        ),
        Index("ix_tax_invoice_lines_pi_line", "proforma_invoice_line_id"),
        Index("ix_tax_invoice_lines_challan_line", "supply_challan_line_id"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    tax_invoice_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("tax_invoices.id", ondelete="CASCADE")
    )
    line_number: Mapped[int] = mapped_column()
    proforma_invoice_line_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("proforma_invoice_lines.id")
    )
    ...
    pi_number_snapshot: Mapped[str | None] = mapped_column(String(50))
    pi_date_snapshot: Mapped[date | None] = mapped_column(Date)
    challan_number_snapshot: Mapped[str | None] = mapped_column(String(50))
    challan_date_snapshot: Mapped[date | None] = mapped_column(Date)
    invoiced_quantity: Mapped[Decimal] = mapped_column(Numeric(18, 4))
